    ...
    {selectedChat ? <>
      ...
      <Box ref={messagesRef} className="chat-inbox-messages" aria-live="polite">
        ...
        {chat?.messages.map((message, index) => {
          ...
          </Box>{!isSystem && writable && <Tooltip title="Ответить"><IconButton className="chat-message-forward" size="small" onClick={() => setReplyTo(message)}><ReplyOutlinedIcon fontSize="small" /></IconButton></Tooltip>}</Box></Fragment>;
        })}
      </Box>
      {writable && <Box component="form" className="request-chat-composer" onSubmit={(event) => { event.preventDefault(); if ((messageText.trim() || pendingImages.length) && !sendMessage.isPending) sendMessage.mutate(); }}>
        {replyTo && <Stack className="chat-reply-composer" direction="row" alignItems="center" spacing={1}><Box minWidth={0} flex={1}><Typography variant="caption" fontWeight={700}>Ответ {senderName(replyTo.sender)}</Typography><Typography variant="caption" noWrap display="block">{replyTo.text}</Typography></Box><IconButton size="small" onClick={() => setReplyTo(null)}><CloseIcon fontSize="small" /></IconButton></Stack>}
